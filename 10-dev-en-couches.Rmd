...
Problématique avec une seule couche : mélange des éléments graphiques et méthier et de la persistance des données  
--> Maintenance corrective et évolutive difficile  
--> Réutilisation du code impossible  
--> Collaboration compliquée

Une solution : les couches logicielles, un ensemble de composants logiciels (ou *couches*) pouvant communiquer entre exécutée

Chaque couche a sa propre responsabilité > **cohésion**  

Un lien entre deux couches est appelé **couplage**. Il peut être plus ou moins fort : 
...
On va décrire dans ce cours quatre couches logicielles :

    1. la couche **IHM** (*interface home-machine*) (\@ref(ihm)) ~front, échange avec l'utilisateur
    2. la couche "métier", découpée en deux couches : 
        - la couche **BLL** (*business logic layer*)  (\@ref(bll)) ~back, modèle de la couche métier (classes et liens entre classes)
        - la couche **BO** (*business object*) (\@ref(bo)) ~back, règles de gestion métier
    3. la couche **DAL** (*data access layer*) (\@ref(dal)) ~data, échange avec ma base de données
...
les grandes lignes du développement en couches. Présentation=IHM, Application = BO & BLL, données = DAL + SGBD. Dans les prijets java ça correspond à un package globalement.

    1. couche IHM (*interface home-machine*) (\@ref(ihm)) ~front, échange avec l'utilisateur
    2. niveau correspondant au "métier", découpé en deux couches : 
        - couche BLL (*business logic layer*)  (\@ref(bll)) ~back, modèle de la couche métier (classes et liens entre classes)
        - couche BO (*business object*) (\@ref(bo)) ~back, règles de gestion métier
    3. DAL (*data access layer*) (\@ref(dal)) ~data, échange avec ma base de données
