...
Méthologie :
    ...
    - Importer la classe (\@ref(import-classe)) : `import package.souspackage.Classe;`
    - Déclarer une variable de type objet (\@ref(declare-classe)) : `Classe varDeTypeObjet ...`
    - Créer une instance d'objet grâce au constructeur (\@ref(instance-classe)) : `... = new Classe(liste de parametres potentiels);`
...
La méthode `main()` est la classe principale : c'est le point d'entrée du programme et la première classe exécutée. 
...
Un constructeur est méthode très particulière. Elle s'appelle toujours comme la classe et peut prendre soit aucun paramètre, soit un ou plusieurs (notion de *surcharge* (\@ref(surcharge))).
...
Les méthodes de classe s'applique à la classe en soi, ce qui signifie que toutes les instances accède ????  
...
Chat gandhi = new Chat("Gandhi",4,"Axel") // Trois parametre : prenom, age, maitre
...
Un objet `String` est **immmuable** : une fois construit, il n'est plus modifiable. Chaque action effectuée sur une `String` induit donc la création d'une nouvelle instance.
